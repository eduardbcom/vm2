...
const requireFileCache = host.requireFileCache;
...
return ((vm, host) => {
		...
		[".js"](module, filename, dirname) {
			if (vm.options.require.context !== 'sandbox') {
				...
				try {
					...
					if (!(filename in requireFileCache)) {
						requireFileCache[filename] = fs.readFileSync(filename, "utf8")
					}

					contents = requireFileCache[filename];
	...
	const _prepareRequire = function(current_dirname) {
		if (vm.options.require.path) {
			current_dirname = pa.resolve(vm.options.require.path);
		}
