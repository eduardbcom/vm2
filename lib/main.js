...
class NodeVM extends EventEmitter {
	...
	runInNewContext(code, filename) {
		if (this.options.compiler !== 'javascript') {
			code = _compileToJS(code, this.options.compiler, filename);
		}

		const { context, internal, prepareRequire } = this._makeNewContext();
		...
		const module = vm.runInContext("({exports: {}})", context, {
			displayErrors: false
		});
		...
		try {
			const closure = script.compile()._compiled.runInContext(context, {
				filename: script.filename,
				displayErrors: false
			});

			var returned = closure.call(context, module.exports, prepareRequire(dirname), module, filename, dirname);
		} catch (e) {
			throw internal.Decontextify.value(e);
		}

		if (this.options.wrapper === 'commonjs') {
			return internal.Decontextify.value(module.exports);
		} else {
			return internal.Decontextify.value(returned);
		}
	}
	...
	_makeNewContext() {
		...
		const internal = vm.runInContext(`(function(require, host) { ${cf} \n})`, context, {
			filename: `${__dirname}/contextify.js`,
			displayErrors: false
		}).call(context, require, this._host);
		...
		const prepareRequire = closure.call(context, this, this._host, internal.Contextify, internal.Decontextify, internal.Buffer);
		...
		if (this.options.sandbox) {
			...
			for (let name in this.options.sandbox) {
				internal.Contextify.globalValue(this.options.sandbox[name], name);
			}
		}
		...
		return { context, internal, prepareRequire };
	}
}
